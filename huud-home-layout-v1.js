/* HUUD OS — HOME / FIELD RECONCILIATION V2
   Regra estrutural:
   TERRENO ≠ MAR VERDE.
   FINANÇAS = liquidez, terreno, dívidas e amortização.
   QG MV = somente operação Mar Verde Imóveis.
   BLOCO = movimento essencial com hora + resultado.
*/
(function(){
  'use strict';

  const FIN_KEY='HUUD_TERRENO_OP_V1';
  ...
  function renameNavigation(){
    document.querySelectorAll('.nav-pill,.nav-btn').forEach(el=>{
      ...
      else if(text==='TERRENO') el.textContent='FINANÇAS';
      if(text.includes('56 PERGUNTAS')) el.style.display='none';
    });
  }

  function renameLandRoom(){
    const v=document.getElementById('view-land');
    if(!v)return;
  ...
  function migrateBlocks(){
    ...
    const hasModel=Object.values(current).flat().some(b=>b && Object.prototype.hasOwnProperty.call(b,'essential'));
    if(hasModel)return;
    const migrated={};
  ...
  function terrainHTML(){
    ...
    return `<section id="huud-terreno-finance" class="huud-fin-op">
      <div class="huud-fin-op-head"><div><div class="huud-fin-k">FINANÇAS & DÍVIDAS // ENTRADA</div><h2>OPERAÇÃO TERRENO & CAIXA</h2><p>Venda do terreno pessoal. Operação de liquidez independente da Mar Verde Imóveis.</p></div><div class="huud-fin-badge">TERRENO ≠ QG MV</div></div>
      <div class="huud-fin-list">${terrain.map(([id,text])=>`<label class="huud-fin-item ${s[id]?'done':''}"><input type="checkbox" ${s[id]?'checked':''} data-terrain="${id}"><span>${esc(text)}</span></label>`).join('')}</div>
    </section>`;
  }
  ...
  function renderTerrain(){
    ...
    if(!view)return;
    let box=document.getElementById('huud-terreno-finance');
    if(!box){box=document.createElement('div');box.id='huud-terreno-finance';view.insertBefore(box,view.firstChild)}
  ...
  function fixHomeModules(){
    ...
    flow.querySelectorAll('.h5-module,.tactical-module-card').forEach(card=>{
      ...
      if(text.includes('MAR VERDE') || text.includes('TERRENO')){
        const h=card.querySelector('h3,.card-title');
        if(h)h.textContent='QG MV';
        card.dataset.huudRoom='land';
      }
    });
    flow.querySelectorAll('*').forEach(el=>{
      if(el.children.length)return;
      const t=(el.textContent||'').trim();
      if(/^MAR VERDE$/i.test(t))el.textContent='QG MV';
      if(/PRIORIDADE ÚNICA/i.test(t) && /pendência financeira/i.test(el.textContent||'')){
        el.textContent='Operação Mar Verde Imóveis';
      }
    });
  }

  function addStyles(){
    if(document.getElementById('huud-reconcile-v2-css'))return;
    const s=document.createElement('style');s.id='huud-reconcile-v2-css';s.textContent=`
      ...
      .huud-fin-op-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}.huud-fin-k{font:900 .58rem var(--mono);letter-spacing:2px;color:var(--blue-inflow)}
      .huud-fin-op h2{font-size:1.35rem;margin:5px 0}.huud-fin-op p{font-size:.68rem;color:var(--text-muted);line-height:1.45}.huud-fin-badge{font:900 .52rem var(--mono);color:var(--blue-inflow);border:1px solid var(--blue-inflow);padding:7px 9px;border-radius:5px;white-space:nowrap}
      .huud-fin-list{display:grid;gap:7px;margin-top:14px}.huud-fin-item{display:flex;align-items:center;gap:9px;padding:10px;border:1px solid var(--border);background:var(--bg-card-elevated);border-radius:7px;font-size:.7rem;cursor:pointer}.huud-fin-item input{accent-color:var(--blue-inflow)}.huud-fin-item.done{opacity:.45;text-decoration:line-through}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    // O Command Center V2 é carregado depois deste arquivo. O timer garante que
    // a reconciliação final aconteça depois da montagem inicial da Home.
